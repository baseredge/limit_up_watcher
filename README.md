# limit_up_watcher · 涨停猎手

**大A打板神器 — 涨停板排队实时监控库**

打板最怕什么？排进去之后，前面堵了多少单子不知道，封单在不在缩也不知道——等真的成交了，万一是被大卖单砸穿炸进去的，想撤都来不及。
...
> **免责声明**：本库仅用于学术研究与技术交流，不构成投资建议。委托 ID 匹配基于手数推测，非交易所官方数据，存在误判可能。使用者自行承担交易决策后果，请遵守相关法律法规。
...
库通过 D201 WebSocket 订阅涨停买一档的逐笔排队数据。涨停价位上的每一笔委托（谁排了、谁撤了、谁被吃了）都会实时推送。

**2. 维护排队队列**

库在内存中维护一个按委托 ID 升序排列的队列，记录当前涨停价上所有排队委托的手数和 ID。新的委托追加到队尾，撤单/成交的委托从队列中移除。

**3. 推测你的委托 ID**

这是关键一步。排队队列中的委托 ID 是交易所分配的递增序号——

- 你调用 `queue(hand_count=100)` 报告排板时，库记住**当前队列最后一个 ID**
- 之后新出现的委托中，如果某笔**手数与你报告的一致**（且 ID > 记录的末位 ID），库就推测这笔是你的
- 比如你排了 100 手，随后队尾新增了一笔 100 手的委托，那大概率就是你

**4. 实时追踪位置**

一旦匹配到你的 ID，库就开始计算：

- **前面** = 队列中所有 ID 小于你的委托的总手数/金额
- **后面** = 队列中所有 ID 大于你的委托的总手数/金额
- 每次数据更新：新增委托 → 判断 ID 在你前还是后，累加对应侧；撤单/成交 → 二分查找，从对应侧扣除
- 如果你的 ID 对应的手数降到 0，推测你已成交

> **局限**：ID 匹配基于手数推测，当同一时刻同价位有多笔相同手数的委托时可能误判（如恰好也有别人排了 100 手）。这不构成精确的成交回报，应以券商实际回报为准。
...
- 需要先在本地安装 [达塔接口](https://lty.gt.tc/dapi/)，启动后登录即可
