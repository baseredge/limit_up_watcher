...
本库仅用于**学术研究、技术交流与学习目的**，不构成任何投资建议或交易指令。

- 本库对 D201 委托数据的解析（包括委托 ID 匹配、位置估算、成交推测）均为基于公开协议的算法推断，**非交易所或券商的官方数据**，存在误判可能
- 使用者应自行承担因使用本库产生的任何交易决策后果，作者不对任何直接或间接损失负责
- 本库不提供交易执行功能，所有交易操作由使用者通过自身券商完成
- 请遵守相关法律法规及证券交易所规则
